...
> 📌 **状态（2026-09-07）**：SEO 机制说明仍有效；文中示例 URL 的域名已从 GitHub Pages 变更为当前自托管地址（`backend/.env` 的 SITE_URL，IP/端口变更后用 `scripts/update-site-url.sh` 更新并重建，sitemap 会自动跟着变）。阅读时把示例域名替换为 SITE_URL 即可。
