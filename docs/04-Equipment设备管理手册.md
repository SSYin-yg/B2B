...
> ⚠️ **历史文档（2026-09-07 标注）**：字段说明与发布流程仍有效；「改 slug/删除设备」的生效机制已更新——后台变更约 3 秒自动重建，删除的页面会自动清理（PRUNE），详见 18-日常使用手册 §2.1。
> 当前权威文档：[17-系统架构文档](./17-系统架构文档.md)、[18-日常使用手册](./18-日常使用手册.md)。
...
> ⚠️ 此脚本为早期 SQLite 时期的一次性工具，**现已废弃**（当前为 PostgreSQL，且脚本硬编码本机路径不可用）。批量更新请走后台编辑 + 自动重建。
```

（已废弃，保留原文仅供历史参考：此脚本曾直接操作 SQLite，按 docx 文件中的产品介绍段落批量更新 `desc_zh` 和 `features_zh`。）
