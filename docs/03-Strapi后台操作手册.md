...
> ⚠️ **历史文档（2026-09-07 标注）**：后台日常操作步骤仍有效；文中「本地开发/SQLite」相关表述已过时，数据库现为 PostgreSQL，后台经 http://<IP>/admin/ 访问。
...
# 升级（需谨慎，先备份 PostgreSQL 数据库）
npm run upgrade
```

⚠️ **升级前必须备份数据库**：`bash scripts/backup-db.sh`（备份到 `backups/db/`）。
