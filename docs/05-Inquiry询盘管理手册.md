...
> ⚠️ **历史文档（2026-09-07 标注）**：接口说明仍有效；新增 resend 重发接口（x-retry-key 鉴权）与 Maillog 邮件日志，详见 18-日常使用手册 §5 与 17-系统架构文档 §8。
...
### Q5：Inquiry 表数据丢失？
A5：从 `backups/db/` 的备份恢复 PostgreSQL（命令见 [10-生产环境部署手册 §八](./10-生产环境部署手册.md)）。
