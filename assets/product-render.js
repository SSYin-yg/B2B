...
(function () {
  ...
  // 草稿预览页：/preview/<slug>.html 与 /equipment/<slug>.html 同深度；
  //           /preview/en/<slug>.html 与 /en/equipment/<slug>.html 同深度
  var isPreview = location.pathname.indexOf('/preview/') === 0;
  var isEnPage = location.pathname.indexOf('/en/equipment/') !== -1 ||
    (isPreview && location.pathname.indexOf('/preview/en/') === 0);
  var basePath = isEnPage ? '../../'
    : (location.pathname.indexOf('/equipment/') !== -1 || isPreview ? '../' : '');
  ...
  function findProduct(id) {
    /* {{SLUG}} 经 encodeURIComponent 注入，这里统一解码后再比对 */
    var key = decodeURIComponent(String(id || ''));
    ...
    var found = list.find(function (x) { return x.id === key; }) || null;
  ...
  function render(product) {
    ...
    if (thumbsEl && mainEl) {
      bindGallery(product, thumbsEl, mainEl);
    }

    window.__currentProduct = { en: product.en, cn: product.cn };
  }

  /* Gallery 缩略图切换（静态页与兜底渲染共用；静态 DOM 中缩略图与主图已由 build-pages 直出） */
  function bindGallery(product, thumbsEl, mainEl) {
    thumbsEl = thumbsEl || document.getElementById('galleryThumbs');
    mainEl = mainEl || document.getElementById('galleryMain');
  ...
  /* 右侧 Contents 目录：移动端折叠 + 滚动高亮（H2/H3 自动收录，标题改动后随静态重建更新） */
  function bindToc() {
    ...
    var heads = Array.prototype.slice.call(document.querySelectorAll('.article .art-h[id]'));
    if (!heads.length || !('IntersectionObserver' in window)) return;
    var visible = [];
    var obs = new IntersectionObserver(function (entries) {
      entries.forEach(function (en) {
        var idx = visible.indexOf(en.target.id);
        if (en.isIntersecting) { if (idx === -1) visible.push(en.target.id); }
        else if (idx !== -1) visible.splice(idx, 1);
      });
      var current = visible.length ? visible[0] : null;
      links.forEach(function (a) {
  ...
  function init(id) {
    ...
    /* 静态生成页：#productRoot 内已有 build-pages 直出的最终 HTML（长正文/TOC/相关设备），
       JS 不再重建正文，只绑定交互；仅旧动态壳（root 无静态内容）时才客户端渲染兜底 */
    var product = findProduct(id);
    var rootEl = document.getElementById('productRoot');
    var hasStatic = !!(rootEl && rootEl.querySelector('.product-layout'));
    if (!hasStatic) {
      render(product);
    } else if (product) {
      window.__currentProduct = { en: product.en, cn: product.cn };
    ...
    langBtns.forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        ...
        if (isPreview) {
          /* 草稿预览页内切换语言：保留 doc/token 查询参数，继续预览另一种语言的草稿 */
          var q = location.search || '';
          location.href = targetLang === 'en'
            ? '/preview/en/' + encodeURIComponent(slug) + '.html' + q
            : '/preview/' + encodeURIComponent(slug) + '.html' + q;
          return;
        }
        if (targetLang === 'en') {
    ...
    document.addEventListener('minelink:lang', function () {
      /* 静态页语言切换走整页跳转（见上方 lang-btn 处理），不做客户端重渲染 */
      var rootEl2 = document.getElementById('productRoot');
      if (rootEl2 && rootEl2.querySelector('.product-layout')) return;
