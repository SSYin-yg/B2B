/**
 * 设备数据中心（由 scripts/build-pages.js 从 Strapi 自动生成，请勿手工编辑）
 * 编辑设备请前往 Strapi 后台 → Equipment，然后重新运行: npm run build-pages
 */
window.MinelinkEquipment = [
  {
    "id": "belt-conveyor",
    "en": "Belt Conveyor",
    "cn": "皮带输送机",
    "type": "screening",
    "images": [
      "assets/images/equipment/belt_conveyor_b0f20e1b27.jpg"
    ],
    "desc_zh": "矿山常用带式输送机，可按产能与布置定制长度、带宽与振",
    "desc_en": "Standard belt conveyors customized for capacity, length, width and incline.",
    "features_zh": [
      "带宽与长度可定制",
      "可配防护与清扫",
      "适配破碎筛分线"
    ],
    "features_en": [
      "Custom width & length",
      "Guards & cleaners optional",
      "Fits crushing lines"
    ],
    "specs": [
      {
        "k_zh": "带宽",
        "k_en": "Belt width",
        "v": "500–1400 mm"
      },
      {
        "k_zh": "输送能力",
        "k_en": "Capacity",
        "v": "按设计"
      },
      {
        "k_zh": "倾角",
        "k_en": "Incline",
        "v": "0–18° 常见"
      }
    ]
  },
  {
    "id": "circular-screen",
    "en": "Circular Vibrating Screen",
    "cn": "圆振动筛",
    "type": "screening",
    "images": [
      "assets/images/equipment/1786346084_b3b489_opt_09bdf065a0.webp"
    ],
    "desc_zh": "圆振动筛是矿山分级主力，筛孔可选、处理能力大。",
    "desc_en": "Circular vibrating screen — workhorse for mine and quarry classification.",
    "features_zh": [
      "大处理量",
      "筛孔可配",
      "多层分级"
    ],
    "features_en": [
      "High throughput",
      "Configurable apertures",
      "Multi-deck"
    ],
    "specs": [
      {
        "k_zh": "层数",
        "k_en": "Decks",
        "v": "2–3"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "100–900 t/h"
      }
    ]
  },
  {
    "id": "crawler-cone-mobile-crushing-station",
    "en": "Crawler Cone Mobile Crushing Station",
    "cn": "履带式圆锥破移动站",
    "type": "mobile",
    "images": [
      "assets/images/equipment/Crawler_Cone_Mobile_Crushing_Station_2ba0e018cd.webp"
    ],
    "desc_zh": "履带式圆锥移动破碎站是一种以圆锥破碎机为核心的移动式破碎设备，采用履带式行走装置，可根据施工现场需要灵活移动和转场。设备主要用于矿石和岩石的中碎、细碎作业，适用于矿山、采石场、砂石骨料生产以及大型工程建设等领域。\n其核心圆锥破碎机通过破碎壁与轧臼壁之间的挤压作用对物料进行连续破碎，适合处理硬度较高的岩石和矿石，可作为移动破碎生产线中的中细碎设备。",
    "desc_en": "Tracked cone plant for secondary/tertiary stages in full mobile lines.",
    "features_zh": [
      "破碎性能",
      "搭载圆锥破碎机,处理量大,效率高,适合中硬至硬质物料的细碎作业。",
      "灵活机动",
      "履带式底盘设计,适应性强,能够快速部署至各种地形复杂的作业现场。",
      "环保节能",
      "采用节能电机和环保设计,减少能耗和噪音,符合环保要求。"
    ],
    "features_en": [
      "Works with jaw & screen plants",
      "Stable product shape",
      "Tracked positioning"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 240 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "100–350 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "160–250 kW"
      }
    ]
  },
  {
    "id": "crawler-impact-crusher",
    "en": "Crawler Impact Crusher Mobile Crushing Station",
    "cn": "履带式反击破移动站",
    "type": "mobile",
    "images": [
      "assets/images/equipment/Crawler_Impact_Crusher_Mobile_Crushing_Station_f37a9d5d79.webp"
    ],
    "desc_zh": "履带式反击式移动破碎站是一种以反击式破碎机为核心的移动破碎设备，采用履带式底盘，具有灵活移动、快速转场和较强场地适应能力。设备主要利用高速旋转转子对物料进行冲击破碎，适用于矿山、采石场、砂石骨料生产、道路建设以及建筑垃圾处理等应用场景。\n反击式破碎机通过转子上的板锤对进入破碎腔的物料进行高速冲击，使物料与反击板之间发生多次碰撞和破碎，在实现物料破碎的同时兼顾成品整形，适合对成品粒度和粒型有较高要求的生产项目。",
    "desc_en": "Tracked impact plant that self-propels into position on rough terrain.",
    "features_zh": [
      "破碎能力",
      "采用颚式破碎技术，具备优秀的物料破碎性能，适用于多种硬度物料的破碎作业。",
      "灵活机动性.",
      "搭载智能的控制系统，支持远程监控及自动化操作，显著提升作业效率。",
      "模块化配置",
      "可根据用户需求，灵活配置不同的破碎、筛分、输送设备，以满足多样化的生产需求。"
    ],
    "features_en": [
      "Self-propelled tracks",
      "Rough-site capable",
      "Good product shape"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 600 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "150–350 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "200–315 kW"
      }
    ]
  },
  {
    "id": "crawler-jaw",
    "en": "Crawler Jaw Crusher",
    "cn": "履带式颚破移动站",
    "type": "mobile",
    "images": [
      "assets/images/equipment/1786929059_34bf44_0e74488882.webp"
    ],
    "desc_zh": "履带式颚式移动破碎站是一种以颚式破碎机为核心的移动式破碎设备，采用履带式行走装置，具有良好的移动性和场地适应能力。设备可根据不同施工环境灵活转场，适用于矿山、采石场、路桥建设、城市建设以及建筑垃圾处理等领域。\n其核心颚式破碎机采用挤压破碎原理，通过动颚与定颚之间的相互作用对物料进行破碎，具有较强的粗碎能力，可有效处理大块岩石、矿石、混凝土及建筑废料。",
    "desc_en": "Tracked jaw plant for primary crushing at the face — cuts haul costs in mines and quarries.",
    "features_zh": [
      "突出的移动性和灵活性",
      "采用履带式行走装置的履带式颚式移动破碎站，能够轻松穿越崎岖山地、泥泞湿地以及狭窄城市空间等各 种复杂地形。这种设计不仅减少了对场地的依赖，还显著提升了施工效率和降低了运输成本。同时，履带式行走装置的 原地转向功能，使得设备即便在狭小空间内也能灵活作业，进一步增强了其适应性和灵活性。",
      "高能的破碎能力",
      "核心部件--颚式破碎机,采用曲动挤压型工作原理,通过电动机驱动皮带和皮带轮,使动颚上下运动,对物 料进行有效压碎。这种破碎方式不仅破碎比大,而且产品粒度均匀,能够满足各种工程项目的需求。此外,颚式破碎机还 具备调整方便的特点,用户可以根据实际生产需要调整破碎速度、啮合角和冲程等参数,以达到很好的破碎效果。",
      "使用场景",
      "路桥建设和城市建设"
    ],
    "features_en": [
      "Tracks for rough terrain",
      "Large primary jaw opening",
      "Inline with screen & cone plants",
      "Hydraulic drive for precise positioning"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 800 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "150–500 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "160–250 kW"
      },
      {
        "k_zh": "整机重量",
        "k_en": "Weight",
        "v": "约 45–65 t"
      }
    ]
  },
  {
    "id": "crawler-lmpact-mobile-crushing-station",
    "en": "Crawler Impact Mobile Crushing Station",
    "cn": "履带式冲击移动破碎站",
    "type": "mobile",
    "images": [
      "assets/images/equipment/1786929059_71518e_fdd39acd4f.webp"
    ],
    "desc_zh": "履带式冲击移动破碎站是一种集给料、破碎、输送和履带行走等功能于一体的移动式破碎设备。设备以冲击式破碎单元为核心，利用高速旋转的转子和板锤对物料进行冲击破碎，可根据不同施工项目灵活移动和转场。\n该设备特别适合需要现场破碎、快速转场以及建筑垃圾资源化处理的项目，可广泛应用于采石场、砂石骨料生产、道路建设、城市改造和建筑垃圾回收等领域。",
    "desc_en": "Tracked impact station for limestone and medium-hard rock with primary shaping.",
    "features_zh": [
      "冲击破碎",
      "采用冲击破碎原理，能够迅速将物料破碎至所需粒度，提高生产效率。",
      "灵活移动",
      "配备履带行走装置，可以方便地在各种复杂地形和作业现场进行移动和转移，适应性强。",
      "智能控制",
      "集成智能控制系统，可实现远程监控和调节破碎参数，操作简便，安全可靠。"
    ],
    "features_en": [
      "Tracked mobility",
      "Aggregate shaping",
      "Flexible relocation"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 700 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "150–400 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "200–315 kW"
      }
    ]
  },
  {
    "id": "crusher-hammer-heads",
    "en": "Crusher Hammer Heads",
    "cn": "破碎机锤头",
    "type": "parts",
    "images": [
      "assets/images/equipment/1785738857_a80c4f_opt_e99febac8f.webp"
    ],
    "desc_zh": "锤式破碎机锤头，高铬或复合材质，提高抗冲击与耐磨寿命。",
    "desc_en": "Hammer crusher heads in high-chrome or composite alloys for impact and wear life.",
    "features_zh": [
      "高铬 / 复合可选",
      "抗冲击耐磨",
      "可按机型配套"
    ],
    "features_en": [
      "High-chrome / composite",
      "Impact & wear resistant",
      "Model-matched"
    ],
    "specs": [
      {
        "k_zh": "材质",
        "k_en": "Material",
        "v": "高铬 / 复合"
      },
      {
        "k_zh": "适用",
        "k_en": "Fits",
        "v": "锤破 / 重锤破"
      }
    ]
  },
  {
    "id": "crusher-hammer-shaft",
    "en": "Crusher Hammer Shaft",
    "cn": "破碎机锤轴",
    "type": "parts",
    "images": [
      "assets/images/equipment/1786700463_373435_3b0ee9db9b.webp"
    ],
    "desc_zh": "破碎机锤轴等核心旋转件，可按图纸或样品定制材质与热处理。",
    "desc_en": "Crusher hammer shafts and rotating parts — material and heat treatment to drawing or sample.",
    "features_zh": [
      "可按图定制",
      "材质与热处理可选",
      "适配主流机型"
    ],
    "features_en": [
      "To drawing/sample",
      "Material & HT options",
      "Fits common models"
    ],
    "specs": [
      {
        "k_zh": "材质",
        "k_en": "Material",
        "v": "按工况"
      },
      {
        "k_zh": "供货方式",
        "k_en": "Supply",
        "v": "成品 / 毛坯"
      }
    ]
  },
  {
    "id": "crusher-liner",
    "en": "Crusher Liner",
    "cn": "破碎机衬板",
    "type": "parts",
    "images": [
      "assets/images/equipment/1786690286_10bc95_opt_04e6cbe273.webp"
    ],
    "desc_zh": "颚板、轧臼壁、破碎壁等耐磨衬板，高锰钢或合金材质可选。",
    "desc_en": "Jaw plates, concaves, mantles and other wear liners in Mn steel or alloy.",
    "features_zh": [
      "高锰钢 / 合金可选",
      "适配多品牌机型",
      "可按样板复制"
    ],
    "features_en": [
      "Mn steel / alloy",
      "Multi-brand fit",
      "Pattern matching"
    ],
    "specs": [
      {
        "k_zh": "材质",
        "k_en": "Material",
        "v": "Mn13 / Mn18 / 合金"
      },
      {
        "k_zh": "适用范围",
        "k_en": "Fits",
        "v": "颚破 / 圆锥破等"
      }
    ]
  },
  {
    "id": "c-type-jaw-crusher",
    "en": "C Type Jaw Crusher",
    "cn": "C 型颚式破碎机",
    "type": "crushing",
    "images": [
      "assets/images/equipment/1786440267_jaw_c_57e553bb3c.webp"
    ],
    "desc_zh": "C 型颚破优化了腔型与机架，破碎效率更高，适合中大型固定生产线。",
    "desc_en": "C-type jaw with optimized chamber and frame for higher efficiency on fixed plants.",
    "features_zh": [
      "优化腔型",
      "效率更高",
      "适合固定线"
    ],
    "features_en": [
      "Optimized chamber",
      "Higher efficiency",
      "Fixed-plant duty"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 1200 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "65–1590 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "75–400 kW"
      }
    ]
  },
  {
    "id": "double-rotor-sand-making-machine",
    "en": "Double-Rotor Sand Making Machine",
    "cn": "双转子制砂机",
    "type": "crushing",
    "images": [
      "assets/images/equipment/1786345751_db3f30_opt_8dc9006c97.webp"
    ],
    "desc_zh": "双转子结构提高冲击破碎与整形效率，适合机制砂与精品骨料。",
    "desc_en": "Dual-rotor design for efficient sand making and premium aggregate shaping.",
    "features_zh": [
      "双转子高效整形",
      "机制砂产量高",
      "可调出料细度"
    ],
    "features_en": [
      "Dual-rotor shaping",
      "High manufactured-sand output",
      "Adjustable fineness"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 30 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "20–280 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "110–640 kW"
      }
    ]
  },
  {
    "id": "double-spiral-sand-washer",
    "en": "Double Spiral Sand Washer",
    "cn": "双螺旋洗砂机",
    "type": "washing",
    "images": [
      "assets/images/equipment/1786439348_4fe367_opt_11da8fe72e.webp"
    ],
    "desc_zh": "双螺旋洗砂机清洗能力强，适合含泥较高的砂石料。",
    "desc_en": "Double-spiral washer for higher clay content sand and aggregate.",
    "features_zh": [
      "清洗能力强",
      "适合高含泥",
      "与脱水筛配套"
    ],
    "features_en": [
      "Strong washing",
      "High-clay feeds",
      "Pairs with dewatering screen"
    ],
    "specs": [
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "60–250 t/h"
      },
      {
        "k_zh": "螺旋直径",
        "k_en": "Spiral dia.",
        "v": "750–1500 mm"
      }
    ]
  },
  {
    "id": "double-tooth-roll",
    "en": "Double Tooth Roll Crusher",
    "cn": "双齿辊破碎机",
    "type": "crushing",
    "images": [
      "assets/images/equipment/1786936180_80b2a3_opt_801256714f.webp"
    ],
    "desc_zh": "双齿辊破碎机适合中等硬度物料，粒度均匀、过粉碎少，常用于煤矿、水泥原料等。",
    "desc_en": "Double-tooth roll crusher for medium-hard materials with uniform product and low fines.",
    "features_zh": [
      "过粉碎少",
      "粒度均匀",
      "适合煤、焦、石灰石"
    ],
    "features_en": [
      "Low over-crushing",
      "Uniform sizing",
      "Coal, coke, limestone"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 500 mm"
      },
      {
        "k_zh": "出料粒度",
        "k_en": "Product size",
        "v": "50–150 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "70–1500 t/h"
      }
    ]
  },
  {
    "id": "gyratory-crusher",
    "en": "Gyratory Crusher",
    "cn": "旋回破碎机",
    "type": "crushing",
    "images": [
      "assets/images/equipment/1785978230_77a531_c24234950d.webp"
    ],
    "desc_zh": "大型矿山粗碎主力，处理能力大、连续作业稳定，适合硬岩与高产能项目。",
    "desc_en": "Large-scale primary gyratory for hard rock and high-capacity continuous duty.",
    "features_zh": [
      "大产能粗碎",
      "适合硬岩",
      "连续稳定运行"
    ],
    "features_en": [
      "High primary capacity",
      "Hard-rock capable",
      "Continuous duty"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 1350 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "600–3200 t/h"
      },
      {
        "k_zh": "适用场景",
        "k_en": "Application",
        "v": "大型露天矿"
      }
    ]
  },
  {
    "id": "heavy-hammer-crusher",
    "en": "Heavy Hammer Crusher",
    "cn": "重锤式破碎机",
    "type": "crushing",
    "images": [
      "assets/images/equipment/1785737072_c921f2_3c783465e8.webp"
    ],
    "desc_zh": "重锤破可一次完成粗、中碎，适合硬度不高的物料，结构紧凑、投资较低。",
    "desc_en": "Heavy hammer crusher combines primary and secondary duties for softer rock with a compact layout.",
    "features_zh": [
      "一机多级破碎",
      "结构紧凑",
      "适合中软物料"
    ],
    "features_en": [
      "Multi-stage in one unit",
      "Compact footprint",
      "Medium-soft materials"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 1200 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "100–3000 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "132–1250 kW"
      }
    ]
  },
  {
    "id": "linear-vibrating-screen",
    "en": "Linear Vibrating Screen",
    "cn": "直线振动筛",
    "type": "screening",
    "images": [
      "assets/images/equipment/1785748244_2338ad_c8d49767bd.webp"
    ],
    "desc_zh": "直线轨迹振动筛，适合分级与脱水，筛分效率高、结构简洁。",
    "desc_en": "Linear-motion screen for classification and dewatering with high efficiency.",
    "features_zh": [
      "直线振动轨迹",
      "分级 / 脱水",
      "维护方便"
    ],
    "features_en": [
      "Linear motion",
      "Classifying / dewatering",
      "Easy service"
    ],
    "specs": [
      {
        "k_zh": "层数",
        "k_en": "Decks",
        "v": "1–3"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "按规格"
      }
    ]
  },
  {
    "id": "multi-cylinder-hydraulic-cone-crusher",
    "en": "Multi-Cylinder Hydraulic Cone Crusher",
    "cn": "多缸液压圆锥破碎机",
    "type": "crushing",
    "images": [
      "assets/images/equipment/1785737413_76bad4_95fa92ee87.webp"
    ],
    "desc_zh": "多缸液压圆锥破，破碎力强、过铁保护可靠，广泛用于中细碎与整形。",
    "desc_en": "Multi-cylinder hydraulic cone with strong crushing force and reliable tramp protection.",
    "features_zh": [
      "多缸液压保护",
      "成品粒形优",
      "适合中细碎"
    ],
    "features_en": [
      "Multi-cylinder protection",
      "Excellent shape",
      "Secondary/tertiary"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 350 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "110–1200 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "250–630 kW"
      }
    ]
  },
  {
    "id": "pe-series-jaw-crusher-standard",
    "en": "PE Series Jaw Crusher (Standard)",
    "cn": "PE 系列颚式破碎机（标准型）",
    "type": "crushing",
    "images": [
      "assets/images/equipment/1784169288_a4354d_5449b8ddad.webp"
    ],
    "desc_zh": "经典固定式颚破，结构可靠、备件通用，适合矿山与采石场粗碎。",
    "desc_en": "Classic fixed jaw crusher with proven structure and common wear parts for primary duty.",
    "features_zh": [
      "结构成熟可靠",
      "备件通用易购",
      "适合硬岩粗碎"
    ],
    "features_en": [
      "Proven structure",
      "Common wear parts",
      "Hard-rock primary"
    ],
    "specs": [
      {
        "k_zh": "进料口",
        "k_en": "Feed opening",
        "v": "600×900 ~ 1500×1800 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "80–1000 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "75–315 kW"
      }
    ]
  },
  {
    "id": "shaping-crusher",
    "en": "Shaping Crusher",
    "cn": "整形破碎机",
    "type": "crushing",
    "images": [
      "assets/images/equipment/1785737245_268d8f_13e13a3bb3.webp"
    ],
    "desc_zh": "用于改善骨料粒形，提高针片状控制与成品外观，常接在二级破碎之后。",
    "desc_en": "Improves aggregate cubicity and reduces flaky particles after secondary crushing.",
    "features_zh": [
      "优化粒形",
      "降低针片状",
      "可接二级破碎后"
    ],
    "features_en": [
      "Cubicity upgrade",
      "Lower flakiness",
      "After secondary stage"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 300 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "150–1500 t/h"
      }
    ]
  },
  {
    "id": "single-cylinder-hydraulic-cone-crusher",
    "en": "Single-Cylinder Hydraulic Cone Crusher",
    "cn": "单缸液压圆锥破碎机",
    "type": "crushing",
    "images": [
      "assets/images/equipment/1785738033_f9dc20_c25df546cf.webp"
    ],
    "desc_zh": "单缸液压结构简洁，排料口调整与清腔方便，适合固定式中细碎线。",
    "desc_en": "Single-cylinder hydraulic cone with simple CSS adjustment and cavity clearing.",
    "features_zh": [
      "液压调排料口",
      "清腔方便",
      "维护成本低"
    ],
    "features_en": [
      "Hydraulic CSS",
      "Easy cavity clear",
      "Lower maintenance"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 560 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "90–1512 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "250–560 kW"
      }
    ]
  },
  {
    "id": "vibrating-dewatering-screen",
    "en": "Vibrating Dewatering Screen",
    "cn": "振动脱水筛",
    "type": "screening",
    "images": [
      "assets/images/equipment/1786439424_c95b83_opt_fde4281e4f.webp"
    ],
    "desc_zh": "用于洗砂后的脱水与细料回收，降低成品含水率。",
    "desc_en": "Dewatering after sand washing to cut product moisture and recover fines.",
    "features_zh": [
      "降低含水率",
      "细料回收",
      "可与洗砂机配套"
    ],
    "features_en": [
      "Lower moisture",
      "Fines recovery",
      "Pairs with sand washers"
    ],
    "specs": [
      {
        "k_zh": "筛面规格",
        "k_en": "Screen size",
        "v": "1.6–11.52 ㎡"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "10–250 t/h"
      }
    ]
  },
  {
    "id": "vibrating-feeder",
    "en": "Vibrating Feeder",
    "cn": "振动给料机",
    "type": "screening",
    "images": [
      "assets/images/equipment/1785737313_c08ccd_c80223ce2b.webp"
    ],
    "desc_zh": "向破碎机均匀给料，可带预筛功能，减少细料进入破碎腔。",
    "desc_en": "Even feed to crushers with optional grizzly to bypass fines.",
    "features_zh": [
      "均匀给料",
      "可选预筛",
      "保护破碎机"
    ],
    "features_en": [
      "Even feed rate",
      "Optional grizzly",
      "Protects crusher"
    ],
    "specs": [
      {
        "k_zh": "给料能力",
        "k_en": "Feed rate",
        "v": "150–3000 t/h"
      },
      {
        "k_zh": "适用设备",
        "k_en": "Pairs with",
        "v": "颚破 / 旋回破等"
      }
    ]
  },
  {
    "id": "vibrating-screen",
    "en": "Vibrating Screen",
    "cn": "振动筛",
    "type": "screening",
    "images": [
      "assets/images/equipment/1785899388_01b923_8ba6727787.webp"
    ],
    "desc_zh": "通用振动筛系列，可按物料与分级要求配置筛面与层数。",
    "desc_en": "General vibrating screen range configurable by material and cut sizes.",
    "features_zh": [
      "多层可选",
      "筛面可换",
      "适配多工况"
    ],
    "features_en": [
      "Multi-deck options",
      "Replaceable decks",
      "Wide duty range"
    ],
    "specs": [
      {
        "k_zh": "层数",
        "k_en": "Decks",
        "v": "2–4"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "70–900 t/h"
      }
    ]
  },
  {
    "id": "vibrating-screen-mesh",
    "en": "Vibrating Screen Mesh",
    "cn": "振动筛网",
    "type": "parts",
    "images": [
      "assets/images/equipment/1785739977_06b9b7_655c2a87f0.webp"
    ],
    "desc_zh": "编织网、冲孔板、聚氨酯筛板等，按孔径与材质配套供货。",
    "desc_en": "Woven wire, punched plate and PU panels — aperture and material as required.",
    "features_zh": [
      "多种筛面形式",
      "孔径可定制",
      "耐磨寿命长"
    ],
    "features_en": [
      "Multiple panel types",
      "Custom apertures",
      "Wear life focus"
    ],
    "specs": [
      {
        "k_zh": "形式",
        "k_en": "Type",
        "v": "编织 / 冲孔 / PU"
      },
      {
        "k_zh": "孔径",
        "k_en": "Aperture",
        "v": "按分级要求"
      }
    ]
  },
  {
    "id": "vsi-impact-crusher",
    "en": "VSI Impact Crusher",
    "cn": "VSI 立轴冲击式破碎机",
    "type": "crushing",
    "images": [
      "assets/images/equipment/1784169293_3041e8_0c91ae33ba.webp"
    ],
    "desc_zh": "立轴冲击破碎机（制砂机），自动更新测试。",
    "desc_en": "Vertical shaft impact crusher for sand making and shaping (rock-on-rock or rock-on-iron).",
    "features_zh": [
      "石打石 / 石打铁可选",
      "机制砂与整形",
      "细度可调"
    ],
    "features_en": [
      "Rock-on-rock / rock-on-iron",
      "Sand & shaping",
      "Adjustable fineness"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 50 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "180–680 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "320–630 kW"
      }
    ]
  },
  {
    "id": "wheel-and-scoop-sand-washer",
    "en": "Wheel-and-Scoop Sand Washer",
    "cn": "轮斗式洗砂机",
    "type": "washing",
    "images": [
      "assets/images/equipment/1785747438_f60cc5_97fb25553e.webp"
    ],
    "desc_zh": "轮斗洗砂机结构简单、耗水相对较低，适合中小型砂场。",
    "desc_en": "Bucket-wheel sand washer — simple structure, moderate water use for mid-size plants.",
    "features_zh": [
      "结构简单",
      "耗水较低",
      "适合中小产能"
    ],
    "features_en": [
      "Simple design",
      "Moderate water use",
      "Mid capacity plants"
    ],
    "specs": [
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "30–300 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "4–15 kW"
      }
    ]
  },
  {
    "id": "wheeled-jaw",
    "en": "Wheeled Jaw Crushing Station",
    "cn": "轮胎式颚破移动站",
    "type": "mobile",
    "images": [
      "assets/images/equipment/1787821485_8e0159_b56acfdc48.webp"
    ],
    "desc_zh": "轮胎式颚破移动站集成给料、颚式破碎与输送，适合采石场与露天矿山快速转场。整机可牵引上路，部署快、产能稳定。",
    "desc_en": "Integrated feeding, jaw crushing and conveying on a wheeled chassis for quick site moves. Ideal for quarries and open-pit operations.",
    "features_zh": [
      "整机可牵引转场，部署周期短",
      "颚腔大开口，适配中硬物料",
      "可选液压调整排料口",
      "模块化设计，便于维护"
    ],
    "features_en": [
      "Towable for fast relocation",
      "Large jaw opening for medium-hard rock",
      "Optional hydraulic CSS adjustment",
      "Modular layout for easy service"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 650 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "80–250 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "110–160 kW"
      },
      {
        "k_zh": "运输尺寸",
        "k_en": "Transport size",
        "v": "约 14 × 2.8 × 3.8 m"
      }
    ]
  },
  {
    "id": "wheeled-screen",
    "en": "Wheel-type Mobile Screening Station",
    "cn": "轮胎式移动筛分站",
    "type": "mobile",
    "images": [
      "assets/images/equipment/Wheel_type_Mobile_Screening_Station_8ba4319241.webp"
    ],
    "desc_zh": "多层级振动筛与给料、输送一体，可独立作业或与移动破碎站串联，实现分级出料。",
    "desc_en": "Multi-deck vibrating screen with integrated feed and conveyors. Works alone or in line with mobile crushers.",
    "features_zh": [
      "2–4 层筛面可选",
      "与移动破碎站灵活串联",
      "轮胎底盘便于转场"
    ],
    "features_en": [
      "2–4 deck options",
      "Easy inline with mobile crushers",
      "Wheeled chassis for relocation"
    ],
    "specs": [
      {
        "k_zh": "筛面层数",
        "k_en": "Decks",
        "v": "2–4"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "100–400 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "30–75 kW"
      }
    ]
  },
  {
    "id": "wheel-type-impact-mobile-crushing-station",
    "en": "Wheel-type Impact Mobile Crushing Station",
    "cn": "轮胎式反击破移动站",
    "type": "mobile",
    "images": [
      "assets/images/equipment/1787821338_93df2a_abd5d5d7bf.webp"
    ],
    "desc_zh": "反击式破碎主机，适合中等硬度物料，成品粒形好，常用于建筑骨料生产线。",
    "desc_en": "Impact crusher plant for medium-hard materials with good cubicity — common in aggregate lines.",
    "features_zh": [
      "成品粒形佳",
      "适合石灰石、混凝土再生",
      "转场部署快"
    ],
    "features_en": [
      "Good product cubicity",
      "Limestone & recycled concrete",
      "Fast deployment"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 500 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "100–300 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "132–200 kW"
      }
    ]
  },
  {
    "id": "wheel-type-multi-cylinder-cone-mobile-crushing-station",
    "en": "Wheel-type Multi-cylinder Cone Mobile Crushing Station",
    "cn": "轮胎式多缸圆锥破移动站",
    "type": "mobile",
    "images": [
      "assets/images/equipment/1787821171_bd400a_67805dfdb2.webp"
    ],
    "desc_zh": "多缸液压圆锥破主机 + 轮胎移动底盘，适用于中细碎与整形，成品粒形好、产量高。",
    "desc_en": "Multi-cylinder hydraulic cone on a wheeled plant for secondary/tertiary crushing with excellent particle shape.",
    "features_zh": [
      "多缸液压，过铁保护可靠",
      "成品粒形优",
      "适合骨料与矿石中细碎"
    ],
    "features_en": [
      "Reliable tramp release",
      "Superior product shape",
      "Secondary/tertiary aggregate & ore"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 215 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "90–380 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "160–250 kW"
      }
    ]
  },
  {
    "id": "wheel-type-single-cylinder-cone-mobile-crushing-station",
    "en": "Wheel-type Single Cylinder Cone Mobile Crushing Station",
    "cn": "轮胎式单缸圆锥破移动站",
    "type": "mobile",
    "images": [
      "assets/images/equipment/1787820893_c1ea4f_dfaba6be15.webp"
    ],
    "desc_zh": "单缸液压圆锥破移动站，结构简洁、维护方便，适合中硬物料的中细碎。",
    "desc_en": "Single-cylinder hydraulic cone plant — simple structure, easy service for secondary crushing.",
    "features_zh": [
      "液压保护与清腔",
      "结构简洁易维护",
      "适合中硬岩"
    ],
    "features_en": [
      "Hydraulic protection & cavity clearing",
      "Simple service access",
      "Medium-hard rock"
    ],
    "specs": [
      {
        "k_zh": "进料粒度",
        "k_en": "Feed size",
        "v": "≤ 230 mm"
      },
      {
        "k_zh": "处理能力",
        "k_en": "Capacity",
        "v": "80–320 t/h"
      },
      {
        "k_zh": "装机功率",
        "k_en": "Power",
        "v": "132–220 kW"
      }
    ]
  }
];
